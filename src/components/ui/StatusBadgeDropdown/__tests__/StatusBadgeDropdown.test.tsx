...
vi.mock('../../Badge', () => ({
  Badge: ({ children, icon, variant, size, shape, interactive, className, ...props }: any) => (
    <span 
      data-testid="badge"
...
describe('StatusBadgeDropdown', () => {
  ...
  describe('Dropdown Functionality', () => {
    ...
    it('does not show current status in dropdown options', async () => {
      ...
      // Current status should not appear in dropdown
      expect(screen.queryByRole('option', { name: /completed/i })).not.toBeInTheDocument();
      
      // Other statuses should appear
      expect(screen.getByRole('option', { name: /plan to watch/i })).toBeInTheDocument();
      expect(screen.getByRole('option', { name: /currently watching/i })).toBeInTheDocument();
    });
  ...
  describe('Custom Configuration', () => {
    it('respects custom availableStatuses prop', async () => {
      ...
      expect(screen.queryByRole('option', { name: /currently watching/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('option', { name: /on hold/i })).not.toBeInTheDocument();
    });
  ...
  describe('Accessibility', () => {
    it('has proper ARIA attributes', () => {
      ...
      expect(trigger).toHaveAttribute('aria-label', 'Current status: Currently Watching. Click to change.');
    });
