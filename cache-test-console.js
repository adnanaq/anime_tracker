  ...
  if (typeof url === 'string') {
    ...
    if (url.includes('api.jikan.moe') || 
      ...
      const requestInfo = {
        ...
        url: url,
        timestamp: timestamp,
        service: url.includes('jikan') ? 'Jikan' : 
                url.includes('myanimelist') ? 'MAL' : 
                url.includes('anilist') ? 'AniList' : 
                url.includes('animeschedule') ? 'AnimeSchedule' : 'Unknown'
      }
      
      requestLog.push(requestInfo)
      console.log(`🚀 API Request #${requestCount}:`, requestInfo.service, '→', url)
    }
  }
...
console.log('✅ Network monitoring active. Try using different dashboard sections!')
...
console.log('1. 🎲 Click "Get Random Anime" button → Should see Jikan API requests')
console.log('2. 🌸 Try SeasonalAnime section → Should see MAL API requests') 
console.log('3. 📅 Try AnimeSchedule section → Should see AnimeSchedule.net requests')
console.log('4. 🔍 Try searching for anime → Should see MAL/AniList API requests')
console.log('')
...
console.log('   🎲 Random Anime: Uses Jikan API directly (cache intentionally bypassed for randomness)')
console.log('   ❌ Seasonal Anime: May use MAL API directly (cache bypassed - needs fixing)')
console.log('   ❌ Anime Schedule: May use AnimeSchedule API directly (cache bypassed - needs fixing)')
console.groupEnd()
