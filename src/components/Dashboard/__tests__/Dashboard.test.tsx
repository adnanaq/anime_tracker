...
vi.mock("../../AuthButton", () => ({
  AuthButton: ({ source }: { source: string }) => (
    <div data-testid="auth-button">Auth Button - {source}</div>
  ),
}));

vi.mock("../../Hero", () => ({
  Hero: ({ anime }: { anime: any[] }) => (
    <div data-testid="hero">Hero - {anime.length} anime</div>
  ),
...
vi.mock("../../AdvancedSearch", () => ({
  AdvancedSearch: () => (
    <div data-testid="advanced-search">Advanced Search</div>
  ),
}));
