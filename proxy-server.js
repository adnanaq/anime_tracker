...
const ANIME_SCHEDULE_TOKEN =
  process.env.ANIME_SCHEDULE_TOKEN || "MDnyW7kcDAMtyiaIg4bc2IIMOrHpbQ";
...
app.post("/mal/oauth/token", async (req, res) => {
  try {
    ...
    const bodyString = Object.keys(req.body)
...
app.listen(PORT, () => {
  ...
  console.log(
    `AniList OAuth proxy: http://localhost:${PORT}/anilist/oauth/token`,
  );
  console.log(
    `AniList GraphQL proxy: http://localhost:${PORT}/anilist/graphql`,
  );
  console.log(
    `AnimeSchedule API proxy: http://localhost:${PORT}/animeschedule/...`,
  );
});
